package SuperHero;//package Superpowers.SuperHero.SuperHuman;

import SuperHero.Affiliation;
 import Superpowers.Human;
 //import Affiliation.affiliation;

public class SuperHuman extends Human {//bc SuperHero.SuperHuman is a subclass of Human

    private String alias; 
    private Affiliation.affiliation affiliation;
    private String ability;
    private String power;
    private String phrase; 


    public SuperHuman(String alias, Affiliation.affiliation affiliation, String ability, String power,String phrase){
        //super(id,name,dateOfBirth,gender,occupation); //importing from human
        this.alias=alias;
        this.affiliation=affiliation; 
        this.ability=ability; 
        this.power=power;
        this.phrase=phrase;

    }
    

	//getters 
    public String getAlias(){
        return alias; 
    }

    public Affiliation.affiliation getAffiliation(){
        return affiliation;
    }

    public String getAbility(){
        return ability; //web powers
    }
     
    //setters 
    public void setAlias(String alias) {
        this.alias = alias;
    }

    public void setAffiliation(Affiliation.affiliation affiliation) {
        this.affiliation = affiliation;
    }

    public void setAbility(String ability) {
        this.ability = ability; //web powers
    }

    public void setPhrase(String phrase){
        this.phrase=phrase; //just your Friendly...
        
    }


    
    public void setPower(String power){
        this.power=power; //pew
    }


    public String getPower(){

        return power; //pew
    }
    
    public String getCatchPhrase(){
        return  phrase;
    }


    @Override
    public String toString(){
        return phrase;
    }
}    